...
const ActionTasks = ({ actionTasks, refetch, setEditedTask, setUpdatedTask }) => {
    ...
    const handleAddTask = () => {
        ...
        fetch("http://localhost:5000/actionTasks", {
            method: "POST",
            headers: {
    ...
    const handleUpVote = (task) => {
        ...
        fetch(`http://localhost:5000/upvoteActionTasks/${task._id}`, {
            method: "PATCH",
            headers: {
                "content-type": "application/json"
            },
    ...
    const handleDelete = (task) => {
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    fetch(`http://localhost:5000/actionTasks/${task._id}`, {
                        method: "DELETE",
                    })
                        .then(res => res.json())
                        .then(data => {
    ...
    return (
        <div>
            <div className='flex justify-center mx-auto items-center mb-4'>
                <h2 className='text-2xl mr-2 font-semibold'>Action Tasks</h2>
                <FiPlusCircle
                        ...
                        <div key={task._id}>
                            {
                                !task?.completed ?
                                    <label
                                        ...
                                        htmlFor="actionTasks"
                                        className='flex items-center justify-center flex-col bg-warning text-white h-[100px] rounded-br-3xl rounded cursor-pointer shadow-2xl'>
                                    ...
                                    <div
                                        key={task._id}
                                        className='bg-warning h-[100px] rounded-br-3xl rounded shadow-2xl'>
                                        <h4
